...
const GAS_API_URL = "https://script.google.com/macros/s/AKfycbxlzRVKr1rJEnXOH2dn6LRkzKi6VqsFT-JhxA3hosmoI2Y0OsdQDFVC3tdLv1Ybhz1p9A/exec";//読み込むスプレットシートを選択するコード
...
const WAITING_LEVEL_MEANINGS: Record<number, { label: string; detail: string }> = {
  1: { label: 'とても空いている', detail:'目安 0分 または営業前(10時以降は基本営業中）' },
...
export default function App() {//アプリを動かすためのコード
  ...
  const getMapImageSources = (buttonName: string): string | null => {//校内マップは WebP だけを読み込みます。
    ...
    const mapImageMap: Record<string, string> = {
      ...
      '屋台': `/屋台村.webp?v=${MAP_ASSET_VERSION}`
    };
    ...
    <div className="space-y-2 pt-2 border-t border-blue-200/60">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 text-xs">
        ...
        <div className="bg-white p-2.5 rounded-xl border border-green-300 shadow-sm flex flex-col items-center text-center hover:border-green-400 transition">
          ...
          <span className="text-[10px] text-slate-400 mt-0.5">目安 0分 または営業前(10時以降は基本営業中）</span>
        </div>
        ...
        <div className="bg-gradient-to-r from-sky-50 to-blue-50 border border-sky-200 rounded-2xl p-4 space-y-3">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-[11px] text-sky-700 mt-1">1. 検索や絞り込みで候補を出す。すべてから気ままにみていくのもよし！ → 2. ピンまたは団体カードをタップ → 3. 団体の場所や状況をチェック！</p>
            </div>
            <button
              onClick={() => {
                if (showGuide) {
